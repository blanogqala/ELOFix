...
- Files: S3-compatible bucket (Cloudflare R2 / AWS S3). Do not rely on the ephemeral Render disk.
...
- `FRONTEND_URL`, `PAYMENT_BASE_URL`, `FRONTEND_BASE_URL`
- `PAYFAST_MERCHANT_ID`, `PAYFAST_MERCHANT_KEY`, `PAYFAST_PASSPHRASE`
- `S3_BUCKET`, `S3_ENDPOINT`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`
- `UPLOAD_ROOT` (optional if using object storage only)
...
1. Create the S3/R2 bucket and enter keys in Render.
