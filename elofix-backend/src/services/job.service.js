...
function normalizeActorRole(actorRole) {
  return String(actorRole || "").trim().toUpperCase();
}

function assertJobActor(job, actorUserId, actorRole, options = {}) {
  if (!job) throw new AppError("Job not found", 404);
  const role = normalizeActorRole(actorRole);
  const actorId = String(actorUserId || "");
  const allowAdmin = options.allowAdmin !== false;
  const allowCustomer = options.allowCustomer !== false;
  const allowProvider = options.allowProvider !== false;

  if (allowAdmin && role === "ADMIN") return;
  if (allowCustomer && role === "CUSTOMER" && String(job.customerId) === actorId) return;
  if (allowProvider && role === "PROVIDER" && String(job.providerId || "") === actorId) return;

  throw new AppError(options.message || "Forbidden", 403);
}

function assertJobCustomer(job, actorUserId, actorRole, options = {}) {
  return assertJobActor(job, actorUserId, actorRole, {
    ...options,
    allowCustomer: true,
    allowProvider: false,
  });
...
function assertJobProvider(job, actorUserId, actorRole, options = {}) {
  return assertJobActor(job, actorUserId, actorRole, {
    ...options,
    allowCustomer: false,
    allowProvider: true,
...
async function updateJobStatus(jobId, status, actorUserId, actorRole) {
  ...
  if (!job) throw new AppError("Job not found", 404);
  assertJobProvider(job, actorUserId, actorRole);
...
async function addUserMaterialSuggestion(jobId, suggested, message, actorUserId, actorRole) {
  const job = await prisma.job.findUnique({ where: { id: jobId }, include: jobInclude });
  if (!job) throw new AppError("Job not found", 404);
  assertJobCustomer(job, actorUserId, actorRole);
  const suggestion = {
    id: randomUUID(),
...
async function acceptUserSuggestion(jobId, suggestionId, actorUserId, actorRole) {
  const job = await prisma.job.findUnique({ where: { id: jobId }, include: jobInclude });
  if (!job) throw new AppError("Job not found", 404);
  assertJobProvider(job, actorUserId, actorRole);
  const meta = await mutateJobMeta(jobId, (m) => {
...
async function rejectUserSuggestion(jobId, suggestionId, actorUserId, actorRole) {
  const job = await prisma.job.findUnique({ where: { id: jobId }, include: jobInclude });
  if (!job) throw new AppError("Job not found", 404);
  assertJobProvider(job, actorUserId, actorRole);
  const meta = await mutateJobMeta(jobId, (m) => ({
...
async function cancelJob(jobId, reason, details, actorUserId, actorRole) {
  const job = await prisma.job.findUnique({ where: { id: jobId }, include: jobInclude });
  if (!job) throw new AppError("Job not found", 404);
  assertJobActor(job, actorUserId, actorRole);
...
async function confirmJobCompletion(jobId, rating, review, actorUserId, actorRole) {
  const job = await prisma.job.findUnique({ where: { id: jobId }, include: jobInclude });
  if (!job) throw new AppError("Job not found", 404);
  assertJobCustomer(job, actorUserId, actorRole, { allowAdmin: false });
...
async function releaseEscrowPayment(jobId, amount, idempotencyKey, requestHash, route, actingUserId, actingRole) {
  const job = await prisma.job.findUnique({ where: { id: jobId }, include: jobInclude });
  if (!job) throw new AppError("Job not found", 404);
  if (normalizeActorRole(actingRole) !== "ADMIN") {
    throw new AppError("Forbidden", 403);
  }
