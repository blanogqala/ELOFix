...
async function submitServicePrice(req, res) {
  ...
  const job = await jobService.submitServicePrice(id, amount, note, req.user.userId, req.user.role);
  res.json({ success: true, job });
}
...
async function acceptUserSuggestion(req, res) {
  const job = await jobService.acceptUserSuggestion(
    req.params.id,
...
async function rejectUserSuggestion(req, res) {
  const job = await jobService.rejectUserSuggestion(
    req.params.id,
    req.params.suggestionId,
    req.user.userId,
    req.user.role
...
async function acceptProposedPrice(req, res) {
  const job = await jobService.acceptProposedPrice(req.params.id, req.user.userId, req.user.role);
  res.json({ success: true, job });
}
