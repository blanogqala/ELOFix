...
function resolveScopedUserId(req, candidate) {
  const ownUserId = String(req.user?.userId || "");
  const requested = candidate != null && String(candidate).trim() !== "" ? String(candidate) : ownUserId;
  if (String(req.user?.role || "").toUpperCase() === "ADMIN") {
    return requested;
  }
  if (requested !== ownUserId) {
