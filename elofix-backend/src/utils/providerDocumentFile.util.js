...
function validateProviderDocumentFileMeta(originalname, mimetype, size) {
  const resolvedExt = normalizeExt(originalname);
  if (!ALLOWED_EXT.has(resolvedExt)) {
    throw new AppError("Document must be PDF, DOC, DOCX, JPG, or PNG", 400);
  }
  const expectedMime = EXT_TO_MIME[resolvedExt];
  const mime = String(mimetype || "").toLowerCase().split(";")[0].trim();
  if (!mime || !ALLOWED_MIME.has(mime)) {
    throw new AppError("Unsupported file type for document upload", 400);
  }
  ...
  if (Number.isFinite(byteSize) && byteSize > 0) {
    if (byteSize > MAX_BYTES) {
      throw new AppError("Document file must be 12MB or smaller", 400);
    }
  }
  return { ext: resolvedExt, mime };
}

async function assertProviderDocumentFileMagic(absolutePath, ext) {
  const handle = await fs.open(absolutePath, "r");
  try {
    const buf = Buffer.alloc(8);
    await handle.read(buf, 0, 8, 0);
    if (!magicMatchesExt(ext, buf)) {
      throw new AppError("File content does not match allowed document types", 400);
    }
  } finally {
    await handle.close();
  }
