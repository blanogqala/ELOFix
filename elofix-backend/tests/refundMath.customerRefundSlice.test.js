...
function testPartialSliceKeepsProcessing() {
  ...
  assert.strictEqual(refund.readyPayoutAmount, 0);
  ...
  assert.deepStrictEqual(refund.finalizedGatewayRefundRefs, ["77"]);
  assert.deepStrictEqual(refund.gatewayRefundRefs, ["77"]);
}
...
function testFinalSliceMarksCompleted() {
  ...
  const { businessComplete, newPending, refund } = applyCustomerRefundSliceToMeta(
    {
      ...
      status: "partial",
      completedAt: null,
      finalizedGatewayRefundRefs: ["77"],
      gatewayRefundRefs: ["77"],
    },
    93,
    now,
    { externalRefundId: "78" }
  ...
  assert.deepStrictEqual(refund.finalizedGatewayRefundRefs, ["77", "78"]);
}
...
function testDoesNotDuplicateFinalizedRefs() {
  const { refund } = applyCustomerRefundSliceToMeta(
    {
      pendingRefund: 186,
      immediateRefund: 0,
    ...
    93,
    "2026-09-18T11:00:00.000Z",
    { externalRefundId: "77", externalRefundIds: ["77", "78"] }
  );
  assert.deepStrictEqual(refund.finalizedGatewayRefundRefs, ["77", "78"]);
  assert.deepStrictEqual(refund.gatewayRefundRefs, ["77", "78"]);
}
...
function main() {
  ...
  testDoesNotDuplicateFinalizedRefs();
