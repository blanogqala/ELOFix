...
import { materialTrackingChecks, trackingLabelsForBatch } from '@/lib/materialBatchTracking';

interface MaterialTrackingMiniProps {
  batch: MaterialBatch | null;
  className?: string;
}

export function MaterialTrackingMini({ batch, className }: MaterialTrackingMiniProps) {
          ...
          <li key={label} className="flex items-center gap-2 text-xs">
            <span
              className={cn(
                ...
                checks[i] ? 'border-primary bg-primary/15 text-primary' : 'border-muted-foreground/30 text-muted-foreground'
              )}
            >
