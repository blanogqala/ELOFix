...
export default function UserProfile() {
  ...
  const [profile, setProfile] = useState<UserProfileState>({ phone: '', profileImage: undefined });
  const [originalProfile, setOriginalProfile] = useState<UserProfileState>({ phone: '', profileImage: undefined });
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const updated = await updateUserProfile(user.id, { phone: profile.phone });
      const saved = { phone: updated.phone, profileImage: updated.profileImage };
      setProfile(saved);
  ...
  const handleImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const updated = await uploadUserAvatar(user.id, compressed);
      setProfile((prev) => ({ ...prev, profileImage: updated.profileImage }));
      setOriginalProfile((prev) => ({ ...prev, profileImage: updated.profileImage }));
      await refreshProfile();
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-4xl space-y-6 md:space-y-8 animate-fade-in">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative mt-1">
                  <Input id="name" value={user?.name || ''} disabled className="bg-muted/50 cursor-not-allowed" />
        ...
        <div className="card-elevated p-4 sm:p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 rounded-lg border border-border hover:bg-muted/50 cursor-not-allowed opacity-60">
        ...
        {hasChanges && (
          <div className="sticky bottom-4 sm:hidden">
            <Button className="btn-accent h-10 w-full whitespace-nowrap" onClick={handleSave} disabled={isSaving}>
              <Save className="mr-2 h-4 w-4" />
              {isSaving ? 'Saving...' : 'Save Changes'}
            </Button>
          </div>
