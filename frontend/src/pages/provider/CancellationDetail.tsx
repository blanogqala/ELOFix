...
import { addDisputeMessage, getDispute, submitProviderDisputeEvidence } from '@/lib/api/disputes';
...
export default function ProviderCancellationDetail() {
  ...
  useEffect(() => {
    ...
    navigate(`/provider/disputes/${id}`, { replace: true });
  ...
  const handleSubmitEvidence = async (payload: { comment: string; images: string[]; videos: string[] }) => {
    if (!id) return;
    const updated = await submitProviderDisputeEvidence(id, payload);
    setDispute(updated);
    toast({ title: 'Response saved', description: 'Your response was updated.' });
  };
      ...
      <div className="mx-auto max-w-4xl space-y-6 animate-fade-in">
        <div className="flex flex-wrap items-center gap-2">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate(dispute.jobId ? `/provider/jobs/${dispute.jobId}` : '/provider/jobs?view=review')}
          >
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back to job
          </Button>
